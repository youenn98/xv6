...
pagetable_t
kernel_pagetable_create()
{
  pagetable_t user_kernel_pagetable = uvmcreate();
  if(!user_kernel_pagetable){
      panic("kernel_pagetable_create");
  }
...
//add a mapping to user's kernel pagetable.
void
uvmmap(pagetable_t ukernel_pagetable,uint64 va,uint64 pa,uint64 sz,int perm)
{
...
int
u2kvmcopy(pagetable_t upage, pagetable_t kpage, uint64 oldsz,uint64 newsz)
{
  pte_t *pteu,*ptek;
  uint64 pa, i;
  uint flags;
  ...
  for(i = oldsz; i < newsz; i += PGSIZE){
    if((pteu = walk(upage, i, 0)) == 0)
      panic("u2kvmcopy: pte should exist");
    if((*pteu & PTE_V) == 0)
      panic("u2kvmcopy: page not present");
    if((ptek = walk(kpage, i, 1)) == 0)
      panic("u2kvmcopy: cannot alloc pte");
    
    pa = PTE2PA(*pteu);
    flags = PTE_FLAGS(*pteu);
    flags &= (~PTE_U);

    *ptek = PA2PTE(pa) | flags;
  }
  return 0;
}
...
void
free_noleaf(pagetable_t pagetable){
  //page table address
  for(int i = 0; i < 512;i++){
      pte_t pte = pagetable[i];
      if(pte & PTE_V){
        pagetable[i] = 0;
        //pte to lower page table
        if((pte & (PTE_R | PTE_W | PTE_X)) == 0){
          uint64 child = PTE2PA(pte);
          free_noleaf((pagetable_t) child);
        }
      }
  }
  kfree((void *)pagetable);
